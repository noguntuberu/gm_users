...
class AccountRecoveryController extends SuperController {
    constructor() {
        super();

        this.model = this.get_model('AccountRecovery');
    }
    ...
    async create_record(data) {
        try {
            ...
            console.log(`[AccountRecoveryController] create_record Error: ${e.message}`);
        }
    }
    ...
    async read_records(conditions, fields_to_return = '', sort_options = '', skip = 0, limit = Number.MAX_SAFE_INTEGER) {
        try {
            ...
            console.log(`[AccountRecoveryController] read_records: ${e.message}`);
        }
    }
    ...
    async update_records(conditions, data_to_set) {
        try {
            ...
            console.log(`[AccountRecoveryController] update_records Error: ${e.message}`);
        }
    }
    ...
    async delete_records(conditions) {
        try {
            ...
            console.log(`[AccountRecoveryController] delete_records Error: ${e.message}`);
        }
    }
}
