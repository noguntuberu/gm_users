...
class TenantController extends SuperController {
    constructor() {
        super();

        this.model = this.get_model('Tenant');
    }
    ...
    async create_record(data) {
        try {
            ...
            console.log(`[TenantController] create_record Error: ${e.message}`);
        }
    }
    ...
    async read_records(conditions, fields_to_return = '', sort_options = '', skip = 0, limit = Number.MAX_SAFE_INTEGER) {
        try {
            ...
            console.log(`[TenantController] read_records: ${e.message}`);
        }
    }
    ...
    async update_records(conditions, data_to_set) {
        try {
            ...
            console.log(`[TenantController] update_records Error: ${e.message}`);
        }
    }
    ...
    async delete_records(conditions) {
        try {
            ...
            console.log(`[TenantController] delete_records Error: ${e.message}`);
        }
    }
}
